...
class Ui_select_injury_type(QMainWindow):
    ...
    def injuries(self, injury_type_selection):
        if injury_type_selection == "Cut":
            self.ui = Ui_step_1(injury_type_selection)
...
class Ui_step_1(QMainWindow):
    def __init__(self):
        ...
        # GET ALL THE WIDGETS TO MODIFY IT WHEN DISPLAYING BASED ON WHAT THE RESPONDER INJURY TYPES SELECTED
        self.title_step = self.findChild(QLabel, "title_step")
        self.gif_player_label = self.findChild(QLabel, "gif_player_label")
        self.text_steps = self.findChild(QLabel, "text_steps")
...
class Ui_step_2(QMainWindow):
    def __init__(self):
        ...
        #self.next_step_button_2.clicked.connect(self.next_procedure)
...
class Ui_step_3(QMainWindow):
    def __init__(self):
        ...
        #self.next_step_button_3.clicked.connect(self.next_procedure)

    def go_back(self):
        window.setCurrentIndex(window.currentIndex()-1)
    
        
class Ui_step_4(QMainWindow):
    def __init__(self):
        super(Ui_step_4, self).__init__()
        loadUi("step_4.ui", self)
        self.go_back_injury_type_4.clicked.connect(self.go_back)
        #self.next_step_button_4.clicked.connect(self.next_procedure)
...
class Ui_gender_patient_window(QMainWindow):
    ...
    def record_session_window(self):
        ...
        print("SELECTED PARTS IF 2 OR MORE INJURIES ARE PRESENT")
...
window.addWidget(window_step_1) # INDEX 9
window.addWidget(window_step_2) # INDEX 10
window.addWidget(window_step_3) # INDEX 11
window.addWidget(window_step_4) # INDEX 12
