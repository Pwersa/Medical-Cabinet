...
# Camera ON or OFF FAILSAFE
camera_on_off = [0]

# CHECK THE INDEX OF CURRENT WINDOW TO KNOW WHAT INDEX WILL GO BACK IF USER WANTS TO GO BACK
check_current_index = []

# CHECK THE INDEX OF CURRENT WINDOW TO KNOW WHAT INDEX WILL GO BACK IF USER WANTS TO CLOSE THE PROGRAM
...
class Ui_scan_qr_code(QMainWindow):
    ...
    def restart_program(self):
        if len(restart_button) >= 5:
            ...
            #subprocess.run(["/home/pi/Desktop/Main_Program/RASPI_TEST_BOOT/config/cabinet_on_start.sh")
            window.refresh()
        else:
...
class Ui_select_body_part(QMainWindow):
    ...
    def old_send(self):
        ...
        try:
            ...
            print("Has Connection, trying to send the file...")
            SEPARATOR = "<SEPARATOR>"
    ...
    def send_to_companion_app(self):
        print("new send is active")
        try:
            print("TRYING TO CHECK INTERNET CONNECTION") 
...
class Ui_enter_injury(QMainWindow):
    ...
    def enter_injury(self):
        if configuration_settings["allow_saving_csv"] == True:
            ...
            #session.clear()
            print("LAMAN NG SESSION >>>>>>>>>>")
            print(session)
            
            session.clear()
            print("DAPAT WALANG LAMAN TO PRE")
            print(session)
        else:
    ...
    def send_injury(self):
        try:
            ...
            s.close()        
            os.remove("/home/pi/Desktop/Main_Program/RASPI_TEST_BOOT/cabinet-history/responder/recorded_accessed_responder" + " " + responder_time[-1] + ".csv")
...
class Ui_gender_patient_window(QMainWindow):
    ...
    def send_to_companion_old(self):
        print("old 2nd send is active")
        try:
            print("TRYING TO CHECK INTERNET CONNECTION") 
            conn = http.client.HTTPConnection(configuration_settings["companion_app_IP"], configuration_settings["port_1st"])                     
            conn.close()
    ...
    def send_to_companion(self):
        try:
            ...
            conn.close()
            time.sleep(1)
            print("FAILED TRYING TO CHECK INTERNET CONNECTION") 
            print("TRYING TO SEND DATA")
            HOST = configuration_settings["companion_app_IP"]
            PORT = configuration_settings["port_2nd"]
            SIZE = 1024
            ...
            # DELETE THE FILES BY DELETING THE FOLDER AND RE-CREATING IT
            parent = "/home/pi/Desktop/Main_Program/RASPI_TEST_BOOT/cabinet-history"
            dir = "session"
...
#window.setWindowFlag(Qt.WindowCloseButtonHint)
