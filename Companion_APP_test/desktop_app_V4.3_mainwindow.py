...
class Ui_Desktop_MainWindow(QtWidgets.QMainWindow):
    ...
    def receive_data(self):
        ...
        os.remove("recorded_accessed_responder" + " " + row[0] + ".csv")
        ...
        def play():
            ...
            try:                
                while True:
                    p = vlc.MediaPlayer("alarmasdasd.mp3")
                    p.play() 
                    sleep(1)
                    p.stop()                    
                    if self.stop_thread:
        ...
        while True:
            ...
            while True:
                ...
                if cmd == "FILENAME":
                    ...
                    conn.close()
                    print("CONNECTED TO DB SESSION")
                    
                    for x in file_name:
                        with open(f'server_folder/cabinet-history/session/{x}', mode ='r')as file:
                        ...
                        sleep(1)
                        os.remove(f'server_folder/cabinet-history/session/{x}')
                        
                    file_name.clear()
                    break
                
            # DISPLAYT ALL SENT DATA
            self.display_data()
            
    def complete_all_responder(self):
        IP = "0.0.0.0"
        PORT = 4699
        SIZE = 1024
        FORMAT = "utf"
        ...
        while True:
            ...
            while True:
                ...
                if cmd == "FILENAME":
                    ...
                    conn.close()
                    print("CONNECTED TO DB RESPONDER")
                    
                    for x in file_name:
                        with open(f'server_folder/cabinet-history/responder/{x}', mode ='r')as file:
                        ...
                        sleep(1.5)
                        os.remove(f'server_folder/cabinet-history/responder/{x}')
    ...
    def start_threading(self):
        ...
        global y
        y = threading.Thread(target=self.check_session)
        y.setDaemon(True)
        y.start()
        
        global z
        z = threading.Thread(target=self.complete_all_responder)
        z.setDaemon(True)
        z.start()
